          ...
          if (PROXY_BASE && songId > 0) {
              ...
              try {
                  ...
                  if (ngText && ngText !== "-1") {
                      ...
                      songUrl = decodeURIComponent((ngMap["10"] || "").trim()) || null;
                      songArtist = (ngMap["4"] || "Unknown").replace(/:$/, "").trim();
                      songTitle = (ngMap["2"] || `Song #${songId}`).replace(/:$/, "").trim();
                  }
              } catch (err) {
                  console.warn("Song info fetch failed", err);
              }

              const _generateCdnUrl = (id) => {
                  ...
                  const md5Cycle = (x, k) => {
                      ...
                      const f = (p, q, r, s, x, sft, t) => { let val = p + (q & r | ~q & s) + x + t; return ((val << sft) | (val >>> (32 - sft))) + q; };
                      const g = (p, q, r, s, x, sft, t) => { let val = p + (q & s | r & ~s) + x + t; return ((val << sft) | (val >>> (32 - sft))) + q; };
                      const h = (p, q, r, s, x, sft, t) => { let val = p + (q ^ r ^ s) + x + t; return ((val << sft) | (val >>> (32 - sft))) + q; };
                      const i = (p, q, r, s, x, sft, t) => { let val = p + (r ^ (q | ~s)) + x + t; return ((val << sft) | (val >>> (32 - sft))) + q; };
                  ...
                  const token = btoa(String.fromCharCode(...hashBytes)).replace(/\+/g, '-').replace(/\//g, '_').replace(/=+$/, '');
              ...
              if (songUrl) {
                  try {
                      decoded = await _tryFetchAudio(songUrl);
                  } catch (e) {
                      console.warn("Newgrounds URL failed, trying CDN fallback:", e.message);
                  }
              }
        ...
        if (levelData.isCustomSong) {
          ...
          window._onlineSongKey    = null;
            ...
            if (ngText && ngText !== "-1") {
                ...
                songTitle = (ngMap["2"] || "Unknown").trim();
                songArtist = (ngMap["4"] || "Unknown").trim();
                songUrl = (ngMap["10"] || "").trim() ? decodeURIComponent(ngMap["10"].trim()) : null;
            }

            const _generateCdnUrl = (songId) => {
            ...
            if (!decoded) {
                try {
                    decoded = await _tryFetchAudio(_generateCdnUrl(levelData.customSongID));
                } catch (e) {
                    console.warn("CDN fallback also failed:", e.message);
                }
            }
